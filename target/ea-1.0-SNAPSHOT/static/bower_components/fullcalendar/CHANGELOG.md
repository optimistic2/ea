...
- list-views auto-height not working (#3346, #4071, thx @WhatTheBuild)
...
- day numbers in month views should be localized (#3339)
...
- when custom views navigates completely into hidden weekends, JS error ([scheduler-375])
...
- fc-content-skeleton DOM element would repeatedly render on navigation in agenda views
...
- Korean typo (#3693)
- fixed switching from any views to listview, eventAfterRender isn't called (#3751)
...
Bugfixes:
- stale calendar title when navigate away from then back to the a views (#3604)
- js error when gotoDate immediately after calendar initialization (#3598)
- agenda views scrollbars causes misalignment in jquery 3.2.1 (#3612)
...
Features:
- `visibleRange` - complete control over views's date range (#2847, #3105, #3245)
...
- `dateAlignment` - custom views alignment, like start-of-week (#3113)
...
- can define a defaultView with a duration/visibleRange/dayCount with needing
  to create a custom views in the `views` object. Known as a "Generic View".

Behavior Changes:
- when custom views is specified with duration `{days:7}`,
  it will no longer align with the start of the week. (#2847)
- when `gotoDate` is called on a custom views with a duration of multiple days,
  the views will always shift to begin with the given date. (#3515)
...
- unwanted scrollbars in month-views when non-integer width (#3453, #3444)
...
Bugfixes:
- list views rendering event times incorrectly (#3334)
- list views rendering events/days out of order (#3347)
...
- DOM structure of month/basic-views day cell numbers changed
...
- height:auto always shows scrollbars in month views on FF (#3202)
- new languages:
	- Basque (#2992)
	- Galician (#194)
	- Luxembourgish (#2979)
...
- Fix week views column title for en-gb ([PR220])
- Publish to NPM ([2447])
...
- IE8 fix: month views renders incorrectly ([2428])
- IE8 fix: eventLimit:true wouldn't activate "more" link ([2330])
- IE8 fix: dragging an event with an href
- IE8 fix: invisible element while dragging agenda views events
...
- views.title wasn't defined in viewRender callback ([2407])
...
- Specify views-specific options using the `views` option. fixes [2283]
- Deprecate views-option-hashes
...
- Bug when rendering business hours and navigating away from original views ([2365])
...
- `aspectRatio`/`height`/`contentHeight` values will be honored *no matter what*
	- If too many events causing too much vertical space, scrollbars will be used ([728]).
	  This is default behavior for month views (**backwards-incompatibility**)
	- If too few slots in agenda views, views will stretch to be the correct height ([2196])
- `'auto'` value for `height`/`contentHeight` options. If content is too tall, the views will
  vertically stretch to accomodate and no scrollbars will be used ([521]).
- Tall weeks in month views will borrow height from other weeks ([243])
- Automatically scroll the views then dragging/resizing an event ([1025], [2078])
- New `fixedWeekCount` option to determines the number of weeks in month views
...
- Android browser cuts off Day views at 8 PM with no scroll bar ([1203])
...
- wrongfully triggering a windowResize when resizing an agenda views event ([1116])
...
- supply views argument in `loading` callback ([1018])
...
- finer snapping granularity for agenda views events ([495], thx @ms-doodle-com)
...
- fixed bug with resizing event to different week in 5-day month views ([740])
...
- bugfixes
	...
	- agenda views left axis sized incorrectly ([465])
	- IE js error when calendar is too narrow ([517])
	- agenda views looks strange when no scrollbars ([235])
...
- lazyFetching option, which can force the calendar to fetch events on every views/date change
- scroll state of agenda views are preserved when switching back to views
- bugfixes
	- calling methods on an uninitialized fullcalendar throws error
	- IE6/7 bug where an entire views becomes invisible ([320])
...
- minTime & maxTime options (restricts shown hours in agenda views)
...
- Important Bugfixes (please upgrade from 1.3!)
	- When current date was late in the month, for long months, and prev/next buttons
	  were clicked in month-views, some months would be skipped/repeated
	- In certain time zones, daylight savings time would cause certain days
	  to be misnumbered in month-views
- Subtle change in way week interval is chosen when switching from month to basicWeek/basicDay views
